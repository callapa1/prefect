...
class Client:
    ...
    def set_key_value(self, key: str, value: Any) -> str:
        """
    ...
    def get_key_value(self, key: str) -> Any:
        ...
        query = {
            "query": {
                with_args("key_value", {"where": {"key": {"_eq": key}}}): {"value"}
            }
        }
        result = self.graphql(query)  # type: Any
    ...
    def delete_key_value(self, key: str) -> bool:
        ...
        result = self.graphql(query=mutation, variables=dict(input=dict(key=key)))

        return result.data.delete_key_value.success

    def list_key_values(self) -> Dict[str, Any]:
        """
        ...
        if prefect.config.backend != "cloud":
            raise ClientError("Key Value operations are Cloud only")
        result = self.graphql(
            {"query": {"key_value(order_by: {key: asc})": {"id", "key", "value"}}}
        )  # type: ignore
        return {res["key"]: res["value"] for res in result.data.key_value}
